...
<head>
    ...
    <script>
        // Apply admin-selected default theme, with safe user override that resets when admin changes default
        (function() {
            var adminDefault = '<?php echo LG_THEME; ?>' || 'light';
            try {
                var storedDefault = localStorage.getItem('lg-theme-default');
                var stored = localStorage.getItem('lg-theme');
                // If admin changed default since last visit, invalidate stored user theme
                if (storedDefault !== adminDefault) {
                    localStorage.removeItem('lg-theme');
                    localStorage.setItem('lg-theme-default', adminDefault);
                    stored = null;
                }
                var theme = stored || adminDefault;
    ...
    <script>
        // Reconcile theme after CSS loads, honoring admin default unless a valid user override exists
        (function() {
            try {
                var adminDefault = '<?php echo LG_THEME; ?>' || 'light';
                var storedDefault = localStorage.getItem('lg-theme-default');
                ...
                var theme = stored || document.documentElement.getAttribute('data-theme') || adminDefault;
                document.documentElement.setAttribute('data-theme', theme);
            } catch (e) {}
        })();
    </script>
                    ...
                    <form method="POST" action="/" autocomplete="off">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="form-check">
                                ...
                                <label for="checkTerms" class="form-check-label">I agree with the <a href="<?php echo htmlspecialchars($templateData['tos_url'] ?? '') ?>" target="_blank">Terms of Service</a></label>
                            </div>
                            <?php endif ?>
